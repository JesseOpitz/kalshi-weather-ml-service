...
def train_model_bundle(
    ...
    *,
    ...
    coverage = float(np.mean((y_test >= np.minimum(test_lower, test_upper)) & (y_test <= np.maximum(test_lower, test_upper))))
    ...
    for index in range(len(test_frame)):
        ...
        test_outcomes.append(
            int(test_bounds_lower[index] <= actual < test_bounds_upper[index])
        )
    ...
    training_examples = (
        train_frame.sample(sample_size, random_state=config.get("random_state", 42))[
            ...
            + [feature for feature in NUMERIC_FEATURES if feature not in {"forecast_mean"}]
        ]
    ...
    bundle = ModelBundle(
        ...
        supported_market_types=sorted(
            train_frame["market_type"].dropna().astype(str).unique().tolist()
        ),
